...
const createEvaluateAllFunction = ({ inDir, outDir: unsafeOutDir, saveChanges }: EvaluateAllOptions) => {
  const outDir = unsafeOutDir || inDir;

  function evaluate<TReturn = void>(pageFunction: () => TReturn): Promise<Record<string, TReturn>>;
  function evaluate<TArg extends SerializableOrJSHandle, TReturn = void>(
    pageFunction: (arg: TArg) => TReturn,
    arg: TArg,
  ): Promise<Record<string, TReturn>>;
  async function evaluate<TArg extends SerializableOrJSHandle, TReturn = void>(
    pageFunction: (arg?: TArg) => TReturn,
    arg?: TArg,
  ): Promise<Record<string, TReturn>> {
...
export default createEvaluateAllFunction;
